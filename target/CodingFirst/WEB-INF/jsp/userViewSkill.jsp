...
<div class="con1">
    ...
    <c:forEach items="${pageBean.pageData}" var="data">
        ...
        <div style=" font-size:  16px !important;font-weight: bold" >问题:${data.skillQuestion}</div>
        <br>

        <div style="background:rgba(255,255,255,0.3) ;border-radius: 6px ; border: 2px solid rgba(255,255,255,0.5)">
            <strong>面试技巧:</strong>${data.skillAnswer}
        </div>
        <%--</form>--%>
    </c:forEach>
    <div class="con2">
        <a  class="btn btn-success btn-sm" href="/faceSkill/showAllInterviewSkillForUser?currentPage=1">第一题</a>
        <a  class="btn btn-success btn-sm"href="/faceSkill/showAllInterviewSkillForUser?currentPage=${pageBean.getCurrentPage()-1}">上一题 </a>
        <a  class="btn btn-success btn-sm"href="/faceSkill/showAllInterviewSkillForUser?currentPage=${pageBean.getCurrentPage()+1}" onclick="disAnswer()">
            下一题
        </a>
        <a  class="btn btn-success btn-sm" href="/faceSkill/showAllInterviewSkillForUser?currentPage=${pageBean.getTotalPage()}">最后一题 </a>
